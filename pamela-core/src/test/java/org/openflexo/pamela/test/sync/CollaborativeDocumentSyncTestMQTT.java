 ...
 * across multiple replicas using MQTT (Artemis).
 *
 ...
 *
 * PREREQUISITES:
 * - An MQTT broker (Artemis) is started embedded for these tests.
 *
 * @author PAMELA Sync Test
 */
public class CollaborativeDocumentSyncTestMQTT {
...
// Start embedded broker
        try {
            ArtemisEmbeddedMQTTBroker.startEmbeddedBroker(MQTT_USERNAME, MQTT_PASSWORD);
        } catch (Exception e) {
            System.out.println("Broker might be already running: " + e.getMessage());
        }
...
// Initialize Replica A
        ...
        syncManagerA = ArtemisMQTTSyncManager.builder()
                .host(MQTT_HOST)
                .port(MQTT_PORT)
...
// Initialize Replica B
        ...
        syncManagerB = ArtemisMQTTSyncManager.builder()
                .host(MQTT_HOST)
                .port(MQTT_PORT)
                .credentials(MQTT_USERNAME, MQTT_PASSWORD)
                .exchangeName(EXCHANGE_NAME)
...
// Skip if Broker is not available
        if (!isBrokerAvailable()) {
            System.out.println("SKIPPING TEST: Broker not available on " + MQTT_HOST + ":" + MQTT_PORT);
            return;
        }
...
// Verify Replica B's document was updated
    ...
    public void testAdderPropagation() throws Exception {
        if (!isBrokerAvailable()) {
            System.out.println("SKIPPING TEST: Broker not available");
            return;
        }
...
// Connect to broker
        System.out.println("[Test] Connecting to MQTT at " + MQTT_HOST + ":" + MQTT_PORT);
        syncManagerA.connect();
...
// ==================================================================================
    ...
    /**
     * Check if Broker is available for testing.
     */
    private boolean isBrokerAvailable() {
        try {
            ArtemisMQTTSyncManager testManager = ArtemisMQTTSyncManager.builder()
                    .host(MQTT_HOST)
                    .port(MQTT_PORT)
                    .credentials(MQTT_USERNAME, MQTT_PASSWORD)
                    .exchangeName(EXCHANGE_NAME)
    ...
    private static class TestOperationListener implements SyncOperationListener {
        ...
        @Override
        public void onConnected() {
            System.out.println(" [Listener] Connected to MQTT");
        }

        @Override
        public void onDisconnected(String reason) {
            System.out.println(" [Listener] Disconnected from MQTT: " + reason);
        }
